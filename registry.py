...
class ClientThread(threading.Thread):
    ...
    def __init__(self, ip, port, tcpClientSocket):
        ...
        # The TCP socket object for communication with the peer.
        ...
        while True:
            try:
                ...
                #   SEARCH  #
                    ...
                    # CREATE_CHAT_ROOM is sent to peer,
                    # if an group with this name already exists
                    if db.is_group_exist(message[1]):
                    ...
                    # if an account with this username is not exist, and the account is created
                        ...
                        db.create_new_chat_room(message[1], message[2])
                        response = "create-success"
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                        self.tcpClientSocket.send(response.encode())
                elif message[0] == "JOIN_GROUP":
                    if db.is_group_exist(message[1]):
                        if db.is_user_in_group(message[2],message[1]) == False:
                            db.add_user_to_chat_room(message[1],message[2])
                        ...
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                        self.tcpClientSocket.send(response.encode())
                elif message[0] == "LIST_CHAT_ROOMS":
                    # Get the list of online users from MongoDB
                    chat_rooms = db.list_all_chat_rooms()
                    # Send the list of online users back to the peer
                    if chat_rooms:
                    ...
                    if db.is_group_exist(message[1]):
                        ...
                        members = db.get_group_members(message[1])
                ...
                elif message[0] == "GET_USER_CHATROOMS":
                    groups=db.get_user_chatrooms(message[1])
                    if groups == []:
                        response = "USER-NOT-JOINING-GROUPS"
                        print("From-> " + self.ip + ":" + str(self.port) + " " + response)
                        logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                        self.tcpClientSocket.send(response.encode())
                    else:
                        response = f"ROOMS {' '.join(groups)}"
                        self.tcpClientSocket.send(response.encode())

                elif message[0] == "CHAT_IN_ROOM":
                    username = message[1]
                    group_name = message[2]
                    if db.user_chat_in_room(username, group_name):
                        response = "DONE"
                    else:
                        response = "REJECT"
                ...
                elif message[0] == "LEAVE-CHATTING_IN-ROOM":
                    username = message[1]
                    if db.user_leave_chatting_in_room(username):
                        response = "DONE"
                    else:
                        response = "REJECT"
                ...
                elif message[0] ==  "LIST_MEMBERS_CHATTING":
                    groupName=message[1]
                    members=db.get_current_chatting_members(groupName)
                    response = f"MEMBERS {' '.join(members)}"
                    self.tcpClientSocket.send(response.encode())
                    print(members)

                elif message[0] == "EXIT_GROUP":
                    db.remove_user_from_chat_room(message[1],message[2])
                    self.tcpClientSocket.send("done".encode())
...
tcpSocket.listen(5)  # In summary, tcpSocket.listen(5) prepares the TCP socket to accept incoming connections
# and sets the maximum size of the connection queue to 5.
# It means that the server can handle up to 5 queued connections at a time before refusing additional connection attempts.
